...
var Tree = function Tree(element, options) {
  ...
  this.$element.off('click.tree.amui.branch').on('click.tree.amui.branch', '.' + this.classes.branchName, $.proxy(function(e) {
  ...
  if (this.options.folderSelect) {
    ...
    if(this.options.add){
        // 添加按钮
        this.$element.find('.'+this.classes.branchName).prepend('<span class="'+this.options.addIcon+'"></span>')
    }
    this.$element.off("click.tree.amui.add").on("click.tree.amui.add",'.'+this.options.addIcon,$.proxy(function(e){
        e.stopPropagation();
        var $el = $(e.currentTarget).closest("li");
        this.$element.trigger('appendbefore.tree.amui', $el);
    },this))
  }
...
Tree.prototype = {
  ...
  selectTreeNode: function selectItem(clickedElement, nodeType) {
    ...
    appendNode:function appendNode(el,type){
        var $el = $(el);
        if(type==="folder"){
            var $el = $(el),
            addflag = true;//增加标志，为真则增,触发渲染
            this.discloseFolder($el,addflag)
        }else{
            return;
        }
  ...
  discloseFolder: function discloseFolder(el,addflag) {
    var $el = $(el);
    var $branch = $el.closest('.' + this.classes.branch);
    var $treeFolderContent = $branch.find('.' + this.classes.branchChildren);
    var $treeFolderContentFirstChild = $treeFolderContent.eq(0);
    this.$element.trigger('beforedisclosedFolder.tree.amui', $branch.data());
    ...
    if(addflag){
    this.populate($treeFolderContent.eq(0));
    }
    if (!$treeFolderContent.children().length&&!addflag) {
  ...
  closeAll: function collapse() {
    ...
    var closedReported = function closedReported(event, closed) {
      ...
      if (self.$element.find('.' + self.classes.branch + '.' + self.classes.open + ':not(".js-hide")').length === 0) {
    ...
    self.$element.find('.' + self.classes.branch + '.' + self.classes.open + ':not(".js-hide")').each(function() {
  ...
  discloseVisible: function discloseVisible() {
    ...
    var $openableFolders = self.$element.find('.' + this.classes.branch + ':not(".' + this.classes.open + ', .js-hide")');
    var reportedOpened = [];
    ...
    self.$element.find('.' + this.classes.branch + ':not(".' + this.classes.open + ', .js-hide")').each(function triggerOpen() {
  ...
  discloseAll: function discloseAll() {
    ...
    var isAllDisclosed = self.$element.find('.' + this.classes.branch + ':not(".' + this.classes.open + ', .js-hide")').length === 0;
